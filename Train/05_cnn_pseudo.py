...
def create_model():
    model = Sequential()
    model.add(Conv1D(filters=32, kernel_size=5, activation="gelu", input_shape=(feature, 1)))
    model.add(MaxPooling1D(pool_size=2))  
    model.add(Conv1D(filters=64, kernel_size=5, activation="gelu"))
    model.add(MaxPooling1D(pool_size=2))
    model.add(Flatten())
...
for fold, (train_index, validate_index) in enumerate(kf.split(X)):
    ...
    model = create_model()
    model.fit(X_train, y_train, batch_size=16, epochs=50, validation_data=(X_validate, y_validate))
    model.save(f'cnn_pseudo_{feature}_model_fold_{fold+1}.h5')
...
fold_result_validate_df.to_csv('cnn_pseudo_{}_fold_results_validate.csv'.format(feature), header=True, index=False)

fold_result_test_df = pd.concat([pd.DataFrame([fold_result_test]) for fold_result_test in fold_results_test])
fold_result_test_df.to_csv('cnn_pseudo_{}_fold_results_test.csv'.format(feature), header=True, index=False)
