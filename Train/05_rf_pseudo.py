...
for fold, (train_index, validate_index) in enumerate(kf.split(X)):
    ...
    joblib.dump(model, f'rf_pseudo_{feature}_model_fold_{fold+1}.joblib')
    ...
    best_fold_results_test.to_csv('rf_pseudo_{}_best_fold_results_test.csv'.format(feature), index=False, sep='\t')
...
fold_result_validate_df.to_csv('rf_pseudo_{}_fold_results_validate.csv'.format(feature), header=True, index=False)

fold_result_test_df = pd.concat([pd.DataFrame([fold_result_test]) for fold_result_test in fold_results_test])
fold_result_test_df.to_csv('rf_pseudo_{}_fold_results_test.csv'.format(feature), header=True, index=False)
