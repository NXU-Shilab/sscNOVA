...
def create_model():
    model = Sequential()
    model.add(Conv1D(filters=16, kernel_size=5, activation="relu", input_shape=(feature, 1)))
...
for fold, (train_index, validate_index) in enumerate(kf.split(X)):
    ...
    model = create_model()
    model.fit(X_train, y_train, batch_size=16, epochs=60, validation_data=(X_validate, y_validate))
    model.save(f'cnn_{feature}_model_fold_{fold+1}.h5')

    # validating dataset evaluation
    y_prob_validate = model.predict(X_validate)
    ...
    best_fold_results_test.to_csv('cnn_{}_best_fold_results_test.csv'.format(feature), index=False, sep='\t')
...
fold_result_validate_df.to_csv('cnn_{}_fold_results_validate.csv'.format(feature), header=True, index=False)

fold_result_test_df = pd.concat([pd.DataFrame([fold_result_test]) for fold_result_test in fold_results_test])
fold_result_test_df.to_csv('cnn_{}_fold_results_test.csv'.format(feature), header=True, index=False)
