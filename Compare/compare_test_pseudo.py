...
data = pd.read_excel('compare_test_pseudo.xlsx')
...
heatmap = sns.heatmap(data_matrix, cmap='coolwarm', annot=True, fmt='.3f', xticklabels=indicators, yticklabels=data.index, 
            linewidths=0.1, center=0.85, vmin=0.6, vmax=1.0, annot_kws={'color':'black'})
...
with PdfPages('compare_test_pseudo.pdf') as pdf:
    pdf.savefig()
    plt.close()
