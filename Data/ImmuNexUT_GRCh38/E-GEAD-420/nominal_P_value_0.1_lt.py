...
for input_file in input_files:
    ...
    # Filter rows where the 12th column (index 11) is less than or equal to 0.1
    filtered_df = df[df.iloc[:, 11] <= 0.1]

    # Select columns 6 to 10 and write the result to an output file
    filtered_df.iloc[:, 5:10].to_csv(f"{input_file.replace('.txt', '_P_value_2.txt')}", sep='\t', index=False)
